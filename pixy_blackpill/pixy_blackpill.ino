...
float Kp = 1.0;
float Ki = 0.2;
float Kd = 0.1; /*왼쪽 바퀴 PDI*/
float Kp_m2 = 1.5;
float Ki_m2 = 0.2;
float Kd_m2 = 0.3; /*오른쪽 바퀴 PDI*/
int e1cnt = 0;
int e1cnt_k = 0, e1cnt_k_1 = 0, d_e1cnt = 0;
int e2cnt = 0;
int e2cnt_k = 0, e2cnt_k_1 = 0, d_e2cnt = 0; /*encoder 현재값,과거값,증감값 저장변수*/
float m1speed = 0;
float m1_ref_spd = 0;
float m1_err_spd = 0;
float m1_err_spd_k_1 = 0;
float m1_derr_spd = 0;
float m1_err_sum = 0; /*모터1 변수*/
float m2speed = 0;
float m2_ref_spd = 0;
float m2_err_spd = 0;
float m2_err_spd_k_1 = 0;
float m2_derr_spd = 0;
float m2_err_sum = 0; /*모터2 변수*/
float ctrl_up = 0;
float ctrl_ui = 0;
float ctrl_ud = 0;
float ctrl_up_m2 = 0;
...
void loop() {
  ...
  if (Serial2.available()) {
   ...
   if (rcv_status) {
    ...
    // 데이터 복사
        memcpy(data.bytes, rx_data, 4);
        slope = data.float_value;
      */
     memcpy((char*)&slope, (char*)rx_data, 4); 
    // 기울기가 양수인 경우, 왼쪽이 빨라지게
    if (slope > 0) {
      m1_ref_spd = 60;
      m2_ref_spd = 30;
    }
    // 기울기가 음수인 경우, 오른쪽이 빨라지게
    else if (slope < 0) {
      m1_ref_spd = 30;
      m2_ref_spd = 60;
    }
